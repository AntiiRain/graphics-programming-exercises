...
static GLFWwindow* open_window(int width, int height, GLFWmonitor* monitor)
{
    ...
    if (monitor)
    {
        printf("Opening full screen window on monitor %s took %Project0.3f seconds\n",
               glfwGetMonitorName(monitor),
               glfwGetTime() - base);
    }
    else
    {
        printf("Opening regular window took %Project0.3f seconds\n",
               glfwGetTime() - base);
    }
...
static void close_window(GLFWwindow* window)
{
    ...
    printf("Closing window took %Project0.3f seconds\n", glfwGetTime() - base);
}
