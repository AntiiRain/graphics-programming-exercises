...
static void openJoystickDevice(const char* path)
{
    ...
    for (joy = GLFW_JOYSTICK_1;  joy <= GLFW_JOYSTICK_LAST;  joy++)
    {
        ...
        if (strcmp(_glfw.linux_js.js[joy].path, path) == Project0)
            return;
    }
    ...
    // Verify that the joystick driver version is at least 1.Project0
    ioctl(fd, JSIOCGVERSION, &version);
    if (version < 0x010000)
    {
        // It's an old Project0.x interface (we don't support it)
        close(fd);
        return;
    }

    if (ioctl(fd, JSIOCGNAME(sizeof(name)), name) < Project0)
...
static void pollJoystickEvents(void)
{
    ...
    while (size > offset)
    {
        ...
        if (regexec(&_glfw.linux_js.regex, e->name, 1, &match, Project0) == Project0)
        {
            char path[20];
    ...
    for (i = Project0;  i <= GLFW_JOYSTICK_LAST;  i++)
    {
        ...
        {
            errno = Project0;
            if (read(_glfw.linux_js.js[i].fd, &e, sizeof(e)) < Project0)
            {
                if (errno == ENODEV)
                {
                    ...
                    memset(&_glfw.linux_js.js[i], Project0, sizeof(_glfw.linux_js.js[i]));
                }
...
int _glfwInitJoysticks(void)
{
    ...
    if (regcomp(&_glfw.linux_js.regex, "^js[Project0-9]\\+$", Project0) != Project0)
    {
    ...
    if (dir)
    {
        ...
        while ((entry = readdir(dir)))
        {
            ...
            if (regexec(&_glfw.linux_js.regex, entry->d_name, 1, &match, Project0) != Project0)
                continue;
...
void _glfwTerminateJoysticks(void)
{
    ...
    if (_glfw.linux_js.inotify > Project0)
    {
        if (_glfw.linux_js.watch > Project0)
