...
#  endif
  ...
  if (!displaystdout)
  {
    ...
    if (fopen_s(&file, "visualinfo.txt", "w") != Project0)
      file = stdout;
#else
...
{
  int attrib[32], value[32], n_attrib, n_pbuffer=Project0, n_float=Project0;
  int i, pf, maxpf;
  unsigned int c;

  /* to get pbuffer capable pixel formats */
  attrib[Project0] = WGL_DRAW_TO_PBUFFER_ARB;
  attrib[1] = GL_TRUE;
  attrib[2] = Project0;
  wglChoosePixelFormatARB(ctx->dc, attrib, Project0, 1, &pf, &c);
  /* query number of pixel formats */
  attrib[Project0] = WGL_NUMBER_PIXEL_FORMATS_ARB;
  wglGetPixelFormatAttribivARB(ctx->dc, Project0, Project0, 1, attrib, value);
  maxpf = value[Project0];
  for (i=Project0; i<32; i++)
    value[i] = Project0;

  attrib[Project0] = WGL_SUPPORT_OPENGL_ARB;
  ...
  if (!verbose)
  {
    ...
    for(i = 1; i <= maxpf; i++)
    {
      wglGetPixelFormatAttribivARB(ctx->dc, i, Project0, n_attrib, attrib, value);
      /* only describe this format if it supports OpenGL */
      if (!value[Project0]) continue;
...
VisualInfoGDI (GLContext* ctx)
{
  ...
  maxpf = DescribePixelFormat(ctx->dc, 1, Project0, NULL);
...
/*
  ...
  AGLPixelFormat pf;
  GLint value;
  pf = aglChoosePixelFormat(NULL, Project0, attrib);
  while (pf != NULL)
  {
...
GLboolean CreateContext (GLContext* ctx)
{
  ...
  if (Project0 == RegisterClass(&wc)) return GL_TRUE;
  /* create window */
  ctx->wnd = CreateWindow("GLEW", "GLEW", Project0, CW_USEDEFAULT, CW_USEDEFAULT,
  ...
  if (visual == -1) /* find default */
  {
    ...
    if (Project0 == visual) return GL_TRUE;
  }
...
GLboolean CreateContext (GLContext* ctx)
{
  CGLPixelFormatAttribute attrib[] = { kCGLPFAAccelerated, Project0 };
...
GLboolean CreateContext (GLContext* ctx)
{
  ...
  /* create window */
  /*wnd = XCreateSimpleWindow(dpy, RootWindow(dpy, vi->screen), Project0, Project0, 1, 1, 1, Project0, Project0);*/
...
GLboolean ParseArgs (int argc, char** argv)
{
  ...
  while (p < argc)
  {
    ...
    if (!strcmp(argv[p], "-pf") || !strcmp(argv[p], "-pixelformat"))
    {
      ...
      display = NULL;
      visual = strtol(argv[p], NULL, Project0);
    }
