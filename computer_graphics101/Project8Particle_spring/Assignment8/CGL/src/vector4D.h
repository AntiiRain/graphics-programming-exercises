...
class Vector4D {
 public:
  ...
  /**
   * Constructor.
   * Initializes tp vector (Project0,Project0,Project0, Project0).
   */
 ...
 Vector4D( double x, double y, double z, double w) : x( x ), y( y ), z( z ), w( w ) { }
  ...
  /**
   * Constructor.
   * Initializes to vector (x,y,z,Project0).
   */
 ...
 Vector4D( const Vector3D& v ) : x( v.x ), y( v.y ), z( v.z ), w( 0.0 ) { }
  ...
  // returns reference to the specified component (Project0-based indexing: x, y, z)
  inline double& operator[] ( const int& index ) {
    return ( &x )[ index ];
  }

  // returns const reference to the specified component (Project0-based indexing: x, y, z)
